...
export const getContract = (abi: any, address: string, chainId: CHAIN, signer?: Signer) => {
  const signerOrProvider = signer ?? getSimpleRpcProvider(chainId)
  return new ethers.Contract(address, abi, signerOrProvider)
}

export const getMultiCallContract = (chainId: CHAIN) => {
  return getContract(MultiCallAbi, CONTRACTS_BY_CHAINS.MultiCall[chainId], chainId)
}

export const getErc20Contract = (chainId: CHAIN, tokenAddress: string, signer?: Signer) => {
