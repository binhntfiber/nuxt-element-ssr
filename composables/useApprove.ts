...
export const useApprove = (options: {
  ...
  const approveDataMap = useState<Record<string, string>>('approveDataMap', () => ({}))
  const { address, APP_CHAIN_ID } = useConnectWallet()
  const shouldApprove = computed(() => {
    return (
      !!address.value && new BigNumber(allowance.value).lt(amount.value) && isFetchedAllowance.value
    )
  })
  const updateAllowance = async (forceUpdate = false) => {
    ...
    } else if (tokenAddress.value && spenderAddress.value && tokenDecimals.value) {
